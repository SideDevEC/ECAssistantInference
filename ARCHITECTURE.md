...
**Addendum 2026-09-26 pm10 (fused-GDN env toggle REMOVED — upstream pinned):** `ECI_DISABLE_GDN` (an env var patched into the llama.cpp submodule) is GONE. Owner decision (option B): the llama.cpp submodule stays PRISTINE upstream (no fork, no local diff) — fused gated-delta-net kernels are permanently pinned `true` by upstream's own default (`cparams.fused_gdn_ar/ch = true` hardcoded in llama-context.cpp). No config knob is exposed; a future need to disable fused-GDN would require a properly vendored fork. Audit 2026-09-26: zero env-var-driven behavior anywhere in ECI product code (zero `getenv` in src/, zero `GetEnvironmentVariable` in csharp/ product code). Smoke 51/51 on the rebuilt dylib (pristine submodule); default behavior byte-identical. History note: intermediate commit 07ce77a carries the never-buildable knob variant (depended on an unpushed submodule patch) — superseded by 1b4b67a, do not build from it; also recorded in README.

**Addendum 2026-09-26 pm9 (plain-chain OOB guard + failure diagnostic):** The no-grammar path in `do_sample_with_grammar` read `arr.data[arr.selected].id` unguarded — when `llama_sampler_dist_apply` receives an empty array it sets `selected = -1`, and the old read was out-of-bounds UB (heap garbage token id, observed as `DECODE_FAILED` on concurrent batched streams; garbage ids fed back into `PromptTokens` also decode-failed follow-up steps). Now guarded: `selected < 0 || >= size` → clean `token = -1` → surfaces as a normal `ECI_ERR_DECODE_FAILED` instead of UB. Added a `[sample-fail]` stderr diagnostic (params + chain state) printed whenever the plain chain returns token<0. Rebuild+redeploy of the Release dylib; E2E batch suite 45/45, benchmarks unchanged within noise (Qwen3-8B: sequential ~87-88, batched x4 ~85-88, grammar ~88 tok/s). The empty-array-at-dist source remains under investigation — the guard makes the failure deterministic and diagnosable instead of silent corruption.
...
**Addendum 2026-09-26 pm6 (pooled selection chain + penalties status):** `do_sample_with_grammar` now fetches a per-thread pooled selection chain keyed by `params_fingerprint` (extended to cover repeat_penalty/penalty_present/repeat_last_n — every chain-affecting param; mismatch = free+rebuild, never wrong params). thread_local destructor frees at exit. Invariants: NO accepts into the pooled chain — token selection identical to before. Measured: batched plain 89.4 vs 89.2 tok/s (noise) — the per-call chain rebuild was NOT the dominant host cost; LLamaSharp's batch advantage is native per-sample sampler work (152k candidates rewrite + partial sort, paid by both engines) plus GPU batching efficiency. **GATED FUTURE OPTION (owner-approved to keep as note):** feed recent_tokens into the pooled chain via reset+replay per call to ACTIVATE repeat/present penalties (today the penalties ring is never populated — penalties are effectively inert; this is a deliberate behavior change requiring LLM-side validation; owner wants less repetition eventually). Smoke 51/51; benchmark tier stats + conformance unchanged.
