...
**Addendum 2026-09-26 pm10 (fused-GDN env toggle → config-injected param):** `ECI_DISABLE_GDN` (an env var patched into the llama.cpp submodule) is GONE. Behavior is pinned + injected: `eci_context_params_t` gains `bool fused_gdn` (default `true`), the submodule exposes `llama_context_params.fused_gdn` (public API field, default `true`, replacing the hardcoded `cparams.fused_gdn_ar/ch = true`), and `eci_create_context` maps param → cparams. C# `ContextConfig.FusedGdn` (default `true`) flows through `EciContextParams`. No env-var-driven behavior anywhere in ECI product code (audit 2026-09-26: zero `getenv` in src/, zero `GetEnvironmentVariable` in csharp/ product code). Smoke 51/51 on the rebuilt dylib; default behavior unchanged.
